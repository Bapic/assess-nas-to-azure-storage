...
function getBlobRecommendationReasons({
  ...
  if (!bestBlobOutcome) {
    return [
      "Azure Blob was selected, but no Blob access tier is currently eligible after applying region and redundancy checks.",
    ];
  }
...
function getFilesRecommendationReasons({
  ...
  if (filesPerformanceEligibility?.scenario === "both") {
    ...
    reasons.push(
      `Source size/IOPS/throughput (${metricsSummary}) fall in the SSD-only suitability range (size < ${formatMetricValue(filesPerfThresholds.maxShareSizeGb)} GB, ${formatMetricValue(filesPerfThresholds.hddMaxIops)} < IOPS < ${formatMetricValue(filesPerfThresholds.filesMaxIops)}, ${formatMetricValue(filesPerfThresholds.hddMaxThroughputMibps)} < throughput < ${formatMetricValue(filesPerfThresholds.filesMaxThroughputMibps)} MiB/s), so only Azure Files Premium SSD remains eligible.`
    );
  }
...
export default function Results({ outcomes, allOutcomes, answers, questions, onRestart }) {
  ...
  return (
    <div className="card results-card">
      ...
      {showRecommendedSection && (
        <section className="recommended-section" aria-label="Recommended services">
